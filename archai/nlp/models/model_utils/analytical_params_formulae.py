...
def _get_hyperparams(model_config: Dict[str, Any]) -> Tuple[int, int, int, List[int], List[int], List[int],int]:
...
def get_params_hf_gpt2_formula(model_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Layer normalization (final)    params['layer_norm'] += 2 * d_model
...
def get_params_hf_gpt_formula(model_config: Dict[str, Any]) -> Dict[str, Any]:
    n_token, tgt_len, d_model, _, d_inner, n_head, d_head, _ = _get_hyperparams(model_config)
...
def get_params_hf_transfo_xl_formula(model_config: Dict[str, Any]) -> Dict[str, Any]:
    n_token, _, d_model, d_embed, d_inner, n_head, d_head, _ = _get_hyperparams(model_config)
...
def get_params_hf_opt_formula(model_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Embedding
    params['embedding'] = n_token * d_model + (tgt_len+2) * d_model

    for _ in range(len(n_head)):
        # Attention
        # QKV
        params['attention'] += d_model * 3 * d_model + d_model * 3 
        # Projection
        params['attention'] += d_model * 3 * d_model + d_model * 3

        # Feed-forward
        params['ff'] += d_model * d_inner[0]*2 + d_inner[0] + d_model

        # Layer normalization
        params['layer_norm'] += 2 *(2 * d_model)

    # Layer normalization (intial layer)
    params['layer_norm'] += d_model * d_model
...
def get_params_hf_ctrl_formula(model_config: Dict[str, Any]) -> Dict[str, Any]:
    n_token, tgt_len, d_model, d_embd, d_inner, n_head, d_head = _get_hyperparams(model_config)
    ...
    # Embedding
    params['embedding'] = n_token*d_embd

    for _ in range(len(n_head)):
        # Attention
        # QKV
        params['attention'] += d_model *3  *d_model + 3*d_model
        # Projection
        params['attention'] += d_model * d_model + d_model
