...
def plot_baseline_and_pareto(path_to_amlt_logs, path_to_baseline_logs, dataset, device_name): 
  ...
  with open(os.path.join(path_to_baseline_logs, device_name, fname), 'rb') as f:       # load pareto memories and latencies
    ...
    print("pareto_logs",pareto_logs)
    print("path_to_baseline_logs",path_to_baseline_logs)
  ...
  print(n_jobs_perconfig,"n_jobs_perconfig")
  print(len(pareto_logs['model_configs']),"len(pareto_logs['model_configs'])")
  for idx in range(len(pareto_logs['model_configs'])):
    ...
    #     break
    print(pareto_train_logs.keys())
    if f'config_{config_idx}_j{job_idx}_train_log.json' not in pareto_train_logs.keys():
      continue
    print(f'config_{config_idx}_j{job_idx}_train_log.json')

    # make sure these are the same models
    this_config = config_to_key(pareto_configs[f'config_{config_idx}_j{job_idx}_model_config.yaml'])
    print(this_config, "this_config")
    print("idx= ",idx)
    orig_config = config_to_key(pareto_logs['model_configs'][idx])
    ...
    all_val_ppls.append(pareto_train_logs[f'config_{config_idx}_j{job_idx}_train_log.json']['valid_ppl'])
    all_configs.append(config_to_key(pareto_configs[f'config_{config_idx}_j{job_idx}_model_config.yaml'], name=f'config_{config_idx}_j{job_idx}'))
  ...
  print(job_keys)
  print(baseline_logs['config_0_j1_train_log.json']['valid_ppl'])
  job_keys_modified = [k + "_train_log.json" for k in job_keys]
  try:
    baseline_val_ppls = np.asarray([baseline_logs[k]['valid_perplexity'] for k in job_keys_modified])
  except:
    baseline_val_ppls = np.asarray([baseline_logs[k]['valid_ppl'] for k in job_keys_modified])
  job_keys_modified2 = [k + "_model_config.yaml" for k in job_keys]
  print(baseline_configs['config_0_j1_model_config.yaml'])
  baseline_configs = np.asarray([config_to_key(baseline_configs[k], name=job_keys[job_keys_modified2.index(k)]) for k in job_keys_modified2])
  print(baseline_configs)
